...
% *** 05.04.2026 (C3 <-> C3') and remove repeating Lure's form equations***
...
Consider the Hopfield neural network (HNN) in the form \eqref{eq:special}, where $g(x)=(g_1(x_1),\ldots,g_n(x_n))^\top$ is the componentwise activation
...
\eqref{eq:special} in the standard Lur\'e form \eqref{eq:ylg-general-system}
...
\begin{equation}\label{eq:HNN-incl}
...
the differential inclusion \eqref{eq:HNN-incl}.
...
\end{enumerate}
% From now on we will write the system \eqref{eq:HNN-incl} of differential inclusions in the form of \eqref{sys:origin} but using symbols from \eqref{eq:HNN-main} and keeping in mind that it is indeed a system with origin as an equilibrium point:
...
\end{equation}
with $P=-B$, $q=T$, $r^{\top}=I_n$ and $\varphi=\overline{\mathrm{conv}}\,g$ as
