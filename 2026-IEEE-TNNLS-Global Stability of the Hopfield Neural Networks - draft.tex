...
and the dynamics is more faithfully modeled as a \emph{Filippov differential inclusion} \cite{p2}.
...
$F$, nonempty compact convex values, and linear growth), every initial condition admits at least one global
...
\eqref{eq:HNN-shifted} satisfies almost all hypotheses of Theorem~3.1 in
Chapter~3 of YLG \ref{thm:YLG-adapted-faithful} and therefore has a pointwise globally stable
...
condition \eqref{eq:ylg-integral} of Theorem~3.1 is required only for indices with
...
Theorem~3.1 in Chapter~3 of YLG are satisfied.  Therefore the system \eqref{eq:HNN-shifted} is  quasi-gradient-like.  Since it's equilibrium point is unique then $\Lambda=\{0\}$ is a singleton, then by Definition~\ref{quasi_gradient_like} 
