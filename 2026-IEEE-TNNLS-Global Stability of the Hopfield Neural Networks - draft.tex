...
\eqref{eq:special} in the standard Lur\'e form \eqref{eq:lure-extended}
\begin{equation*}
  \dot x = P x + q\,\xi, 
  \qquad \sigma = r^{*}x, 
...
\begin{align*}
  \Delta_i
  &= \frac{1}{2}\bigl((s^\star+\delta)^* Q (s^\star+\delta)
      - (s^\star)^* Q s^\star\bigr) - h^* \delta \\
  &= s^{\star^*} Q \delta + \frac{1}{2} \delta^* Q \delta - h^* \delta.
\end{align*}
Using $\delta_j = 0$ for $j \neq i$ and $\delta_i = -2 s_i^\star$, we obtain
\[
  s^{\star^*} Q \delta = (Q s^\star)_i \delta_i,
  \qquad
