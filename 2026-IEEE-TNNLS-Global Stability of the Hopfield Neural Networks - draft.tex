...
\emph{discontinuous, possiblydunbounded} activations. 
...
avoiding smoothness assumptions and extending classical bounded results. 
...
Classical Lyapunov analyses that assume smooth, bounded nonlinearities are therefore
...
% exterior methods. Extending GAS guarantees beyond the bounded class is thus essential
...
We consider continuous-time HNNs with componentwise activations that may be discontinuous and unbounded.
...
\smallskip\noindent
\textit{(iii).}
By Definition~\ref{G_n}, each $g_i$ in the class $\mathcal G'$ is piecewise continuous, sign–preserving and nondecreasing with a finite number
of first–kind discontinuities on any compact interval and strict¶
upward jumps at the discontinuity points. Its Filippov
convexification $\varphi_i=\overline{\operatorname{conv}}\,g_i$ is therefore a nondecreasing piecewise single–valued multifunction.  In particular,
...
$\varphi_i$ is nondecreasing and has at most linear growth (by
...
For item~3, we performed the equilibrium shift $z=x-e$ and rewrote the
