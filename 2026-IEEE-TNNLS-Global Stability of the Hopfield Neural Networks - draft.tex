...
\newtheoremstyle{myplain}  % имя стиля
  {\topsep}                 % отступ сверху
  {\topsep}                 % отступ снизу
  {\normalfont}             % шрифт тела теоремы (прямой)
  {}                        % отступ для заголовка
  {\bfseries}               % шрифт заголовка (жирный)
  {.}                       % разделитель после заголовка
  {5pt plus 1pt minus 1pt} % пробел после заголовка
  {}                        % спецификатор заголовка (пусто)
...
\newtheorem*{assumption1}{Assumption C1}
\newtheorem*{assumption2}{Assumption C2}
\newtheorem*{assumption3}{Assumption C3}
\newtheorem*{assumption3mat}{Assumption C3'}
...
we derive three verifiable conditions --- linear growth, componentwise one-sided Lipschitz, 
and \emph{frequency condition} --- under which the network admits a \emph{unique equilibrium} 
that is \emph{globally asymptotically stable}. 
The proof leverages the Yakubovich--Leonov--Gelig quadratic-plus-integral functional, 
...
and the dynamics is more faithfully modeled as a \emph{Filippov differential inclusion} \cite{Filippov1988}.
...
% For bounded discontinuous activations, Forti and Nistri established existence of a
% \emph{unique globally asymptotically stable} (GAS) equilibrium for HNN with discontinuous neuron activations by recasting the dynamics as a differential inclusion 
...
% Hardware-friendly and optimization-driven designs often deploy
...
\item \textbf{Three verifiable hypotheses.} 
      ...
      Lipschitz, and a \emph{YLG-type frequency condition} --- and show via a YLG
      quadratic\,+\,integral functional that \emph{existence, uniqueness, and global asymptotic stability (GAS)} of the equilibrium hold.
...
or merely piecewise–continuous activations. In 2003 paper Forti and Nistri established existence of a
\emph{unique globally asymptotically stable} equilibrium for HNN with discontinuous neuron activations~\cite{p1}.
...
&A^{*} &\quad&\text{Transpose of } A.\\
&A^{-1} &\quad&\text{Inverse of } A.\\
&A^S = \tfrac{1}{2}(A + A^{*}) &\quad&\text{Symmetric part of } A.\\
...
\begin{equation}
  ...
  \label{eq:special}
\end{equation}
where $B=\operatorname{diag}(b_1,\dots,b_n)> 0$ collects effective leak rates ($b_j:=1/(R_j C_j)$ after rescaling), $T\in\mathbb{R}^{n\times n}$ is the interconnection matrix (with signs admitting excitatory/inhibitory effects), $g(x)=(g_1(x_1),\ldots,g_n(x_n))^*$ acts componentwise, and $I\in\mathbb{R}^n$ is the (rescaled) input vector. This compact representation makes explicit the split between linear passive dynamics and static nonlinearities, a structure central to absolute–stability analyses.
...
As was presented in \ref{sec:intro}, in many applications the activation function in the right-hand side of system~\eqref{eq:special}
...
we employ A.~Filippov's theory~\cite{Filippov1988}.
...
\[  
    \dot x (t) \in \hat f(x(t)) = -Bx(t)+T~\overline{\operatorname{conv}}\big[g(x(t))\big]+I,
    \label{1.2}
\]
\[  
    \hat f(x(t)) = \bigcap\limits_{\delta>0}\bigcap\limits_{\mu(\mathcal{N})=0}\overline{\operatorname{conv}}\big[f(\mathcal{B}(x(t),\delta) - \mathcal{N})\big], 
\]
where $\hat f(x(t))$ is the smallest convex 
...
The above can be rewritten~\cite{ShevitzPaden1994} as:
\begin{equation}\label{eq:special_conv}
\begin{aligned}
&\dot x (t)\in \hat f(x(t)) = \\
	&=\overline{\operatorname{conv}}\big\{\lim f(x_i(t)) ~|~x_i(t) \to x(t),~x_i(t)\notin N_f\big\} = \\
	&= -B x(t)+T~\overline{\operatorname{conv}}[g(x(t))]+I. 
\end{aligned}
\end{equation}
...
Now, using Filippov's definition of solutions, we define what is meant by an equilibrium point of the network and its global asymptotic stability in this theory~\cite{p1}.
...
\[      
    0 \in \hat f(e) = 
    -B e + T~\overline{\operatorname{conv}}\big(g(e)\big) + I.
\]
...
\begin{definition}
    Set-valued map $\hat f(p)$ is called \textit{upper semicontinuous} in $p_0$, if for any open $V\subset\mathbb{R}^n$, $\hat f(p_0)\subset V$, there exists a neighborhood $U(p_0)$ of $p_0$ such as:
\[ 
\hat f(p) ~\subset ~ V, \quad \forall p\in U(p_0).
\]
\end{definition}
...
\begin{equation}
  \dot x(t) \in \hat f(x(t)), \qquad x(t)\in\mathbb{R}^n,
...
\[
  \Lambda := \{\,c\in\mathbb{R}^n \mid 0\in \hat f(c)\,\}.
\]
\end{definition}
...
{\cite[Defs.~1.10--1.11]{YLG}}]\label{quasi_gradient_like}
System~\eqref{eq:aut-incl-prelim} is called
...
\(r\in\mathbb{R}^{n\times m}\), with a (possibly discontinuous) nonlinearity
...
\begin{definition}[Extended nonlinearity {\cite[Sec.~1.2]{YLG}}]\label{extended_nonlinearity}
A pair of functions $(x(\cdot),\xi(\cdot))$ defined on an interval
$[t_0,t_1]$ is called a solution of the Lur'e system if
\begin{itemize}
  \item $x(t)$ is absolutely continuous and $\xi(t)$ is integrable;
  \item the relations from \eqref{eq:lure-extended} 
...
Hopfield network is an ordinary differential
equation \eqref{eq:special}, 
% \[
...
inclusion~\eqref{eq:special_conv}. 
% For brevity we denote the Filippov
...
satisfying $\dot x(t)\in \hat f(x(t))$ almost everywhere
...
\begin{multline}\label{eq:Phi-t}
   ...
   \text{of } \dot x\in \hat f(x),\ x(0)=x_0 \bigr\}.
\end{multline}
Under the basic hypotheses of Filippov theory (upper semicontinuity of
$\hat f$, nonempty compact convex values, and linear growth), every initial condition admits at least one global
...
\begin{remark}
  Upper semicontinuity, nonempty compact convex values and linear growth of the right hand side of \eqref{eq:special_conv}
hold in view of Assumptions~\textbf{C1-C2} wich we will present in Section~\ref{sec:problem}.
\end{remark}
...
considers the family $\mathcal{J}$ of all Filippov trajectories
$x:\mathbb{R}_+\to\mathbb{R}^n$ of~\eqref{eq:special_conv} and checks
that $\mathcal{J}$ is closed under time shifts and concatenation and is
upper semicontinuous with respect to initial data.  Then
$\mathcal{J}$ is a generalized semiflow on $\mathbb{R}^n$, and our main
result, that will be formulated in section~\ref{sec:main}, can be rephrased as saying that, under Assumptions~\textbf{C1-C3}, the
...
differential equation with a measurable ``extended  nonlinearity''.
...
\]
Note that $\eta(t)$ is precisely the extended nonlinearity $\xi(t)$ from Definition \ref{extended_nonlinearity}. Thus the HNN can be viewed as a control system
\(
   \dot x = -Bx + T\eta + I
\)
...
of dynamical–systems and random–dynamical–systems theory (see e.g.~\cite{Caraballo2001}).
...
Consider the system \eqref{eq:lure-extended}
where each \(\varphi_i\) is piecewise
single-valued. Let
\begin{equation}
  \chi(\lambda) := r^{*}(P-\lambda I)^{-1}q
  \label{eq:ylg-transfer}
\end{equation}
be the transfer matrix of \eqref{eq:lure-extended}, and let
...
such that
\begin{equation}
  TM + \operatorname{Re}\bigl[(T+i\omega \Theta)\chi(i\omega)\bigr] \ge 0,
...
\item[(v)]
The stationary set \(\Lambda\) of \eqref{eq:lure-extended} is bounded.
Moreover, if a solution $(x(t),\xi(t))$ of \eqref{eq:lure-extended}
satisfies
\begin{equation}
...
Then system \eqref{eq:lure-extended} is quasi-gradient-like.
...
\(i\)th components of \(r^{*}x\) and \(r^{*}x^0\), respectively.
...
\eqref{eq:lure-extended} is pointwise globally stable.
\end{theorem}
...
Consider the Hopfield neural network in the form \eqref{eq:special}, where $g(x)=(g_1(x_1),\ldots,g_n(x_n))^*$ is the componentwise activation
mapping.
...
at the discontinuity points, and is sign–preserving.  In particular, its Filippov convexification
$\mathrm{conv}\,g_i$ is a piecewise single–valued
multifunction in the sense of YLG.

\begin{assumption1}[\bf Linear growth]\label{ass:C1}
...
\begin{assumption2}[\bf One-sided Lipschitz (componentwise)]\label{ass:C2}
For each $i\in\{1,\dots,n\}$ there exists $C_i\in\mathbb{R}$ such that
for all $x,y\in\mathbb{R}^n$ and for all a.e.\ selections
$\eta\in\overline{\operatorname{conv}}\,g(x)$, $\zeta\in\overline{\operatorname{conv}}\,g(y)$ one has
\[
...
\eqref{eq:special} in the standard Lur\'e form \eqref{eq:lure-extended} with
...
\[
  \varphi(\sigma)
  := \overline{\operatorname{conv}}\,g(\sigma)
  = \bigl(\overline{\operatorname{conv}}\,g_1(\sigma_1),\dots,
          \overline{\operatorname{conv}}\,g_n(\sigma_n)\bigr)^* .
\]
...
% ------------------------------------------------
\begin{assumption3}[\bf YLG-type frequency condition]\label{ass:C3prime}
...
diagonal matrices
\begin{multline*}   
   \mathcal{T}=\operatorname{diag}(\tau_1,\dots,\tau_n)> 0,\\
   \mathcal{M}=\operatorname{diag}(\mu_1,\dots,\mu_n)\geq 0,\\
   \Theta=\operatorname{diag}(\vartheta_1,\dots,\vartheta_n)
\end{multline*}
such that
...
\textup{(i)}–\textup{(ii)} of Theorem~\ref{thm:YLG-adapted-faithful},
...
\begin{assumption3mat}[\bf Diagonal Lyapunov stability (DLS)]\label{ass:C3}
There exists a diagonal matrix $H=\operatorname{diag}(h_1,\dots,h_n) > 0$
and a constant $\alpha > 0$ such that
\begin{equation}\label{eq:C3-matrix}
  -HB - B^{*}H + \alpha\bigl(T^{*}H + HT\bigr) < 0 .
\end{equation}
\end{assumption3mat}
...
\begin{corollary}[from assumption~\textbf{C3$'$}]\label{col_from_C3}
There is a diagonal $\alpha=\operatorname{diag}(\alpha_i>0)$ with $M:=\alpha T+T^{\!*}\alpha<0$ and
    \begin{equation*}
      \frac{4C_i\alpha_i b^{2}_{\max}}{\|M\|_{2}}<b_{\min}\quad(i=1,\dots,n),
    \end{equation*}
where $\lambda_m$ is the eigenvalue with the largest absolute value or spectral radius of $T+T^{\!*}$, and $b_m=\min\limits_{1\leq i \leq n}b_i$, $b_M=\max\limits_{1\leq i \leq n}b_i$.
\end{corollary}

\begin{lemma}[C3 implies C3$'$]\label{lem:C3prime-implies-C3}
Suppose that Assumption~\textbf{C3} holds. Then there exist a
diagonal matrix $H=\operatorname{diag}(h_1,\dots,h_n)> 0$ and a constant
...
of Theorem~\ref{thm:YLG-adapted-faithful} are satisfied for the system
\eqref{eq:lure-extended} with $(P,q,r)=(-B,T,I_n)$.  By the frequency
criterion in YLG (see Eqns.~(3.10)–(3.11) on pp.~114–115), there
exists a symmetric matrix $H_{\mathrm{YLG}}=H_{\mathrm{YLG}}^{*}$
such that the quadratic form
\[
  W(x,\xi)
  := \bigl[2x^{*}H_{\mathrm{YLG}} + \xi^{*}\Theta r^{*}\bigr]\,
     (P x + q\xi)
     + \xi^{*}\mathcal{T}\,(r^{*}x - \mathcal{M}\xi)
\]
satisfies
...
\begin{equation}\label{eq:H-YLG-pd}
  H_{\mathrm{YLG}} > 0 .
\end{equation}
Set $\xi\equiv 0$ in the definition of $W$ and use $r^{*}=I_n$ and
$P=-B$:
\begin{multline*}
  W(x,0)
   = \bigl[2x^{*}H_{\mathrm{YLG}}\bigr]\,(P x)
   = 2x^{*}H_{\mathrm{YLG}}P x = \\
   = -2x^{*}H_{\mathrm{YLG}}B x .    
\end{multline*}
Together with \eqref{eq:W-leq0}, this gives
\begin{equation}\label{eq:HB-negative}
  -2x^{*}H_{\mathrm{YLG}}B x \le 0,
  \qquad \forall\,x\in\mathbb{R}^n.
\end{equation}
Since $B=\operatorname{diag}(b_1,\dots,b_n)> 0$ and
$H_{\mathrm{YLG}}> 0$, we have $x^{*}H_{\mathrm{YLG}}Bx>0$ for
all $x\neq 0$.  Hence \eqref{eq:HB-negative} is in fact strict for
$x\neq0$, and the symmetric matrix
\[
  S_{\mathrm{YLG}}
  := -H_{\mathrm{YLG}}B - B^{*}H_{\mathrm{YLG}}
\]
is negative definite:
\begin{equation}\label{eq:SYLG-negative}
  x^{*}S_{\mathrm{YLG}}x
  = -2x^{*}H_{\mathrm{YLG}}B x < 0,
  \qquad \forall\,x\neq 0 .
\end{equation}
Define the diagonal matrix
\begin{equation}\label{eq:H-diag-def}
  H := \operatorname{diag}\bigl((H_{\mathrm{YLG}})_{11},
                         \dots,
                         (H_{\mathrm{YLG}})_{nn}\bigr).
\end{equation}
By \eqref{eq:H-YLG-pd}, all diagonal entries $(H_{\mathrm{YLG}})_{ii}$
are strictly positive, so $H> 0$.  Since $B$ is diagonal and
positive definite, for every $x\neq 0$ we have
\begin{align*}
  x^{*}\bigl(-HB - B^{*}H\bigr) x
    &= -2x^{*}HBx \\
    &= -2(B^{1/2}x)^{*}\bigl(B^{1/2}HB^{1/2}\bigr)(B^{1/2}x) \\
    &< 0,
\end{align*}
because $B^{1/2}HB^{1/2}> 0$.  Thus
...
$S(0)=S_0< 0$ by \eqref{eq:S0-negative}.  Hence there exists
$\alpha'>0$ such that $S(\alpha)< 0$ for all
$\alpha\in[0,\alpha']$.  Choosing any
$\alpha\in(0,\alpha']$, we obtain
\[
  -HB - B^{*}H + \alpha\bigl(T^{*}H + HT\bigr)
  = S(\alpha) < 0 ,
\]
...
In the previous section we have rewritten  Hopfield network
in the Lur'e form \eqref{eq:lure-extended} after identifying the state,
...
Consider the continuous–time Hopfield neural network as in \eqref{eq:special}
\begin{equation*}
  \dot x(t) = -B x(t) + T\,g(x(t)) + I, \qquad x(t)\in\mathbb{R}^n,
\end{equation*}
where
\begin{itemize}
  \item $B = \operatorname{diag}(b_1,\dots,b_n)> 0$ is a diagonal matrix
        of leak rates;
  \item $T\in\mathbb{R}^{n\times n}$ is the interconnection (synaptic) matrix;
  \item $I\in\mathbb{R}^n$ is a constant input vector;
  \item $g=(g_1,\dots,g_n)^{*}:\mathbb{R}^n\to\mathbb{R}^n$ is the
...
activation functions (Definition~\ref{G_n}) and that Assumptions
\textbf{C1}–\textbf{C3} hold.

Interpret system~\eqref{eq:special} in the sense of Filippov, i.e.\
as the differential inclusion \eqref{eq:special_conv}
\begin{equation*}
   \dot x(t) \in -B x(t) + T\,\varphi(x(t)) + I,
   \qquad \varphi(x) = \overline{\operatorname{conv}}\,g(x).
\end{equation*}
...
\begin{enumerate}
  ...
  \item we verify that Assumptions~\textbf{C1}, \textbf{C2},
        ...
        Theorem~\ref{thm:YLG-adapted-faithful}, and then invoke that theorem
...
Interpreting \eqref{eq:special} in the sense of Filippov, we obtain
the differential inclusion \eqref{eq:special_conv}.
...
of~\eqref{eq:special_conv} defined on the whole half–line $[0,+\infty)$.
\end{proposition}
...
discontinuities on any compact set (Definition~\ref{G_n}) and $\varphi$ is its
...
least one absolutely continuous solution of~\eqref{eq:special_conv} on
the interval $|t-t_0|\le\tau$, where
$\tau = \min\{\alpha,\rho/c\}$. Hence Filippov solutions of
\eqref{eq:special_conv} exist locally.
...
Therefore, every solution of~\eqref{eq:special_conv} can be continued to
...
at least one Filippov solution $x(\cdot;x_0)$ of~\eqref{eq:special_conv}
...
reverting from $z$ back to $x$, the network is written as the Filippov
differential inclusion
\begin{equation}\label{eq:HNN-shifted}
      \dot x(t)\in P x(t)+q\,\varphi(x(t)), \qquad x(t)\in\mathbb R^n,
\end{equation}
with $P=-B$, $q=T$, $r^{*}=I_n$ and $\varphi=\overline{\operatorname{conv}}\,g$ as
in~\eqref{eq:lure-extended}.  In the notation of YLG this is exactly a
...
\eqref{eq:HNN-shifted} satisfies almost all hypotheses of Theorem~\ref{thm:YLG-adapted-faithful} and therefore has a pointwise globally stable
...
$(P,q,r)=(-B,T,I_n)$ and $\varphi=\overline{\operatorname{conv}}\,g$, all hypotheses
\textup{(i)}–\textup{(vi)} of Theorem~\ref{thm:YLG-adapted-faithful} are satisfied.
\end{lemma}
...
Theorem~\ref{thm:YLG-adapted-faithful} are fulfilled, with $T=\mathcal T$,
$M=\mathcal M$, $\theta=\Theta$ and $m=n$.

\smallskip\noindent
\textit{(iii).}
By Definition~\ref{G_n}, each $g_i$ in the class $\mathcal G'$ is piecewise continuous and sign–preserving with a finite number
of first–kind discontinuities on any compact interval and strict
upward jumps at the discontinuity points. Its Filippov
convexification $\varphi_i=\overline{\operatorname{conv}}\,g_i$ is therefore a sign–preserving multifunction.  In particular,
$\sigma_i\,\xi_i\ge 0$ for all $\xi_i\in\varphi_i(\sigma_i)$.

Choose $\mu_i:=0$ for all $i$.  Then the sector inequality \eqref{eq:ylg-sector} of
Theorem~\ref{thm:YLG-adapted-faithful} reduces to
\[
...
nondegeneracy condition \eqref{eq:ylg-nondeg} becomes
$\sigma_i\notin \mu_i\varphi_i(\sigma_i)$ for $\sigma_i\neq 0$, which
is trivially true when $\mu_i=0$.  Finally, the additional integral
condition \eqref{eq:ylg-integral} of Theorem~\ref{thm:YLG-adapted-faithful} is required only for indices with
...
\[
  P + qM_0^{-1}r^{*}
  = -B + T M_0^{-1},\qquad
  M_0:=\operatorname{diag}(\mu_1^0,\dots,\mu_n^0),
\]
is Hurwitz.  Since $B=\operatorname{diag}(b_i)>0$ and $T$ is fixed, we
...
Hopfield inclusion \eqref{eq:special_conv} admits a \emph{unique}
...
\end{equation}
where $H>0$ is the symmetric matrix obtained from the YLG
frequency inequality (cf.\ Eqns.~(3.10)–(3.11) in Chapter~3 of \cite{YLG})
and $\vartheta_i$ are the diagonal entries of~$\Theta$.  Because each
$\varphi_i$ has at most linear growth (by
...
$x(t)^* H x(t)\equiv\mathrm{const}$.  Hence, if a complete
...
% \smallskip\noindent
% \textit{(vi).}
% With $\mu_i=0$, condition (vi) of Theorem~\ref{thm:YLG-adapted-faithful} reduces to
% \[
...
For item~3, we've performed the equilibrium shift $z=x-e$ and rewrote the
network in the Lur\'e form \eqref{eq:HNN-shifted}.  By
Lemma~\ref{lem:YLG-conditions}, hypotheses (i)–(v) of
Theorem~\ref{thm:YLG-adapted-faithful} are satisfied.  Therefore the system \eqref{eq:HNN-shifted} is  quasi-gradient-like.  Since it's equilibrium point is unique then $\Lambda=\{0\}$ is a singleton, then by Definition~\ref{quasi_gradient_like} 
this means that it is also a gradient-like system therefore the origin is globally asymptotically stable for the
shifted system.  Undoing the shift gives global asymptotic
stability of the equilibrium $e$ for the original Hopfield network
\eqref{eq:special} in the sense of Filippov.
\end{proof}

\begin{corollary}
  Similar result can be proven without the sign–preserving property of activation functions using D.W. Shevitz and B. Paden’s approach (see Appendix~\ref{app:proofs}), however this work is focused on proving stability using frequency methods of YLG~\cite{YLG}.
\end{corollary}
...
\end{cases}
\qquad x=(x_{1},x_{2})^{*}\in\mathbb{R}^{2},
\end{equation}
...
\item \textbf{C3$'$ (diagonal stability):} With $H=I_{2}$ and $\alpha=1$, the matrix inequality
\[
-2I_{2}+(T+T^{*})< 0
\]
holds when $|a|<2$.
...
\begin{proposition}[Nonuniqueness of forward trajectories]
...
\]
which satisfies \eqref{eq:scalarinc} almost everywhere. For each such $\mu$ we obtain a distinct Filippov solution $x(t)=(0,x_{2}(t))^{*}$ of \eqref{eq:example-system} with $x(0)=(0,0)$.
\end{remark}
...
This function has a first-order discontinuity and is clearly not bounded (see Fig. \ref{fig2:left}).
...
Lastly, let's present the counterexample to assumption \textbf{C3$'$} (DLS) by taking a skew-symmetric matrix $T$ ($T^{*} = -T$):

\begin{equation}\label{example2}
\begin{cases}\frac{\mathbf{d}x_1}{\mathbf{d}{t}}=-x_1- g_2(x_2),\\\frac{\mathbf{d}x_2}{\mathbf{d}{t}}=-x_2 + g_1(x_1).\end{cases}
\end{equation}
...
\begin{figure*}[h]
\begin{minipage}{0.49\linewidth}
\includegraphics[width=1.05\textwidth]{Figures/new_g_x.png}
  \caption{Graph for the activation function $g(x)$.}\label{fig2:left}
\end{minipage}
\hfill
\begin{minipage}{0.49\linewidth}
 \includegraphics[width=1.05\textwidth]{Figures/new_g_phase.png}
...
\begin{equation}\label{eq:hnn-qubo}
  ...
  g_i(u)=\mathrm{sign}(u),
\end{equation}
where \(B=\operatorname{diag}(b_i)>0\), \(\beta>0\), and
\begin{equation}\label{eq:Qshift}
  Q_{\mathrm{shift}}:=Q+\mu I,\quad \mu>\lambda^+_{\max}(Q)\ \Rightarrow\ Q_{\mathrm{shift}}>0.
\end{equation}
Then \(T=-\beta Q_{\mathrm{shift}}\) yields \(T+T^*<0\), so \textbf{C3} holds (with \(\alpha=I\)); \textbf{C1-C2} hold for the sign activation.
\begin{theorem}[Global convergence to the planted optimizer]\label{th:hnn-solves-strong-field-qubo}
Under the strong field condition and \textbf{C1-C3}, \eqref{eq:hnn-qubo} has a unique Filippov equilibrium \(x'\), globally asymptotically stable, and \(g(x')=\mathrm{sign}(x')=s'\), the unique QUBO optimizer. 
\end{theorem}
\begin{proof}
  Proof in Appendix~\ref{app:qubo-proofs}.
\end{proof}
...
\end{equation}
then \((Q^{\mathrm{MC}},h)\) meets Definition~\ref{eq:strong-field}. With \(Q_{\mathrm{shift}}=Q^{\mathrm{MC}}+\mu I>0\) and the HNN \eqref{eq:hnn-qubo}, Theorem~\ref{th:hnn-solves-strong-field-qubo} implies:
\begin{corollary}\label{cor:hnn-planted-maxcut}
The HNN output converges globally to \(s'\) aligned with the planted partition \((A,B)\), which is the unique minimizer of the MAX-CUT QUBO with field.
\end{corollary}
\begin{proof}
  Proof in Appendix~\ref{app:qubo-proofs}.
\end{proof}
...
\begin{equation}\label{eq:penalty-V}
  V(x)=\phi(x)+\tfrac12 x^* A x - \ell^* x,\qquad A>0,
\end{equation}
and run \(\dot x\in -\partial V(x)\). This realizes \eqref{eq:lure-extended}
...
Then the unique GAS equilibrium \(x'\) solves \(P(c)\). 
...
\begin{equation}\label{eq:SVM-primal}
  ...
  y_i(w^* z_i + b) \ge 1-\xi_i,\ \xi_i\ge 0
\end{equation}
can be written as an exact-penalty objective
\begin{equation}\label{eq:SVM-penalty}
  q_\rho(u,\xi)=\tfrac12 u^* u + C\sum_i \xi_i + \rho\sum_i \max\{0,\,1-\xi_i-y_i u^*\bar z_i\},
\end{equation}
with \(u=(w^*,b)^*\), \(\bar z_i=(z_i^*,1)^*\), and sufficiently large~\(\rho\).
Let \(x=(u^*,\xi^*)^*\). The piecewise-linear kinks in the hinge term define a componentwise discontinuous activation (single upward jump). Choosing a diagonal leakage and a shifted quadratic synaptic matrix (so \(T+T^*<0\)) yields a discontinuous HNN of the Lur’e form satisfying \textbf{C1-C3}. Therefore:
\begin{proposition}[Unique globally convergent SVM solver]\label{prop:SVM}
The Hopfield network built from \eqref{eq:SVM-penalty} by \(\dot x\in -\partial V(x)\) has a unique Filippov equilibrium \(x'=(u',\xi')\) and \(x(t)\to x'\) from any initialization. 
The classifier parameters \((w',b')\) coincide with the optimal soft-margin SVM solution. 
\end{proposition}
\begin{proof}
...
At each frozen state \(z\), the control \(u'(z)\) is the unique solution of the strictly convex QP
\begin{equation}\label{eq:opt-problem}
  \min_{u\in\mathbb{R}^m}\ J_z(u) := \tfrac12 u^* H u + f(z)^* u
  \quad\text{s.t.}\quad u_{\min}\le u\le u_{\max},
\end{equation}
where \(H>0\) and \(f(\cdot)\) is affine. The solver must operate continuously as \(z\) evolves; multi-valued or initial-state-dependent outcomes are unacceptable.
...
\begin{multline}\label{eq:phi-def}
  \phi(x):=\tfrac12 x^* Hx + \sum_{i=1}^m \iota_{[u_{\min,i},u_{\max,i}]}(x_i),
  \\
  \partial\phi(x)=Hx + N_{[u_{\min},u_{\max}]}(x),
\end{multline}
and define the parameterized energy
\begin{equation}\label{eq:Vz-def}
  V_z(x):=\phi(x)-b(z)^* x+\tfrac12 x^* A x,
\end{equation}
with \(A>0\) diagonal and \(b(z)\) affine in \(z\).
We run the \emph{subgradient flow}
\begin{equation}\label{eq:hnn-subgrad}
  \dot x \in -\partial V_z(x).
\end{equation}
This is a Filippov inclusion with finitely many first-order discontinuities (due to the box normal cone). It fits the Lur’e form \eqref{eq:lure-extended} and satisfies linear growth, one-sided slope and YLG-type frequency (\textbf{C1-C3}) condition under the choices below.

\begin{proposition}[Real-time solver]\label{prop:rt-solver}
Under Assumptions~\textbf{C1--C3} and the model \eqref{eq:Vz-def}–\eqref{eq:hnn-subgrad}, for each frozen \(z\) the HNN admits a unique Filippov equilibrium \(x'(z)\), globally asymptotically stable, and \(x'(z)=u'(z)\) solves \eqref{eq:opt-problem}. 
\end{proposition}
\begin{proof}
  Proof in Appendix~\ref{app:control-proofs}.
\end{proof}
...
\end{equation}
has solution \(u'(z)=\Pi_{[-1,1]}(-\alpha z/h)\).
Choose
\[
...
\]
so \textbf{C1-C3} hold; \(x'(z)=u'(z)\) 
and the closed loop has the unique equilibrium \((z',x')=(0,0)\). 
...
QP:
\begin{multline*}    
  \min_{u\in[-1,1]^2}\ J_z(u)=\tfrac12 u^* H u + q(z)^* u, \\
  H=\operatorname{diag}(2,1),\ 
  q(z)=\begin{bmatrix}z_1+0.5 z_2\\ 2z_2\end{bmatrix}.
\end{multline*}
Then \(u'(z)=\Pi_{[-1,1]^2}\big(-H^{-1}q(z)\big)\).
Let
\begin{multline*}
  \phi(x)=\tfrac12 x^* Hx+\iota_{[-1,1]^2}(x),\\
  V_z(x)=\phi(x)-q(z)^* x+\tfrac12 x^* A_\ell x,\\ 
  A_\ell=\operatorname{diag}(a_1,a_2)>0,
\end{multline*}
and \(\dot x\in -\partial V_z(x)=-(H+A_\ell)x+q(z)-N_{[-1,1]^2}(x)\).
Then \(\dot V_z\le -x^* A_\ell x\), hence \textbf{C1--C3} hold and \(x'(z)=u'(z)\) globally. 
...
serves as the decisive criterion, irrespective of boundedness
of activation functions. For the Hopfield model new results on global stability
in the case of not bounded activation functions,
...
% $\sigma=(\sigma_1,\dots,\sigma_m)^{*}$ and
% $\varphi(\sigma)=(\varphi_1(\sigma_1),\dots,\varphi_m(\sigma_m))^{*}$
...
% such that
% \begin{equation}
%    T M + \operatorname{Re}\bigl[(T+i\omega\theta)\,X(i\omega)\bigr] \;\geq\; 0,
...
% vectors $r^{*}x$ and $r^{*}x^0$, respectively. In the case
...
\]
As was shown in section~\ref{sec:nonunique}, both $x^{(1)}(\cdot)$ and $x^{(2)}(\cdot)$ are absolutely continuous, satisfy \eqref{eq:example-system} almost everywhere and $x^{(1)}(0)=x^{(2)}(0)=(0,0)$, hence are Filippov solutions with the same initial data.
...
generalized Lyapunov method \cite{ShevitzPaden1994} is employed, 
...
Using the generalized Clarke gradient~\cite{p14} and the chain rule for regular functions~\cite{ShevitzPaden1994},
...
Similarly to the way such result was proven in \cite{eq_ref_11, GAS_of_delayed, GAS_discrete_time_varying}, this theorem can be proven using Gronwall’s inequality and the existence and continuation theorems for differential inclusions by A.F. Filippov \cite{Filippov1988}. Therefore such proof mostly depends on the theory of upper semicontinuous set-valued mappings and the fact that the set-valued map from the right hand side of inclusion \eqref{eq:special_conv} satisfies all conditions of Filippov's theorems.
\end{remark}
\begin{IEEEproof}
To prove this theorem, we will use the fact that the existence and continuation of solutions \( x(t) \) of differential inclusion \( \dot{x} \in F(x) \) with the initial condition \( x(0) = x_0 \) on the interval \([0, t_0]\) for \( t_0 > 0 \) is guaranteed if the set-valued mapping \( F(x) \) is upper semicontinuous and has nonempty, convex, and compact values. In other words, it satisfies the so-called basic requirements of A.F. Filippov's theorems on the existence and continuation of solutions \cite[Theorems 1 and 2, pp. 77-78]{Filippov1988}. 

As established in Step~1 of the proof of Proposition~\ref{prop:global-existence}, the set-valued map on the right-hand side of inclusion~\eqref{eq:special_conv}\label{2.2.6} is upper semicontinuous with nonempty, convex, and compact values. With these conditions satisfied, we can apply the theory presented by Filippov \cite{Filippov1988}, which implies that solutions to the system exist, and the existence interval for each  solution of system \eqref{eq:special} with initial data $x(0) = x_0$ is $[0, +\infty)$.
\end{IEEEproof}
...
\begin{lemma}\label{main_lemma}
    \textnormal{Let \textbf{C2} and \textbf{C3$'$} hold, then by collorary~\ref{col_from_C3} there exist positive $d$ and $\varepsilon$ such that}
...
\begin{IEEEproof}
    The set-valued map $\varphi(x)$ from the right-hand side of \eqref{eq:special_conv} is upper semicontinuous with nonempty, convex, and compact values (see the proof of Theorem~\ref{max_int} and Proposition~\ref{prop:global-existence}). 
    ...
    \[K = \{x:~x^{*}x \leqslant R^2 \} = \{ x:~\|x\| \leqslant R\}.\]
Now we determine the tangent cone\footnotemark[2] to $K$:
\[T_K(x) = \{y : y^{*}x \leqslant 0\}, \quad \forall x \in \partial K.\]
...
Let $\gamma \in \overline{\operatorname{conv}}(g(0))$ be a constant vector. Since $\varPhi(\lambda,x)$ is upper semicontinuous and, therefore, a measurable mapping, then the Makarov’s theorem \cite{YLG} implies that if $x(t)$ is a solution of system \eqref{eq:special} on the interval $[t_0,t_1]$, where $t_0<t_1\leqslant+\infty$, then there exists an almost everywhere measurable mapping $\eta(t) \in \overline{\operatorname{conv}}\big(g(x(t))\big)$ such that the equality holds almost everywhere on $[t_0, t_1]$:
...
x^{*} x - d \widetilde{\eta}^{*}  \alpha  Bx \geqslant \varepsilon x^{*} (-B) x . 
\end{equation}

Multiplying equation \eqref{2.3.4} by the same bracket:
\begin{equation}\label{2.3.6}
\begin{aligned}&
\underbrace{(-2B^{-1}x+2d \alpha\widetilde\eta)^{*}}_{=\xi} f  =\\&= \underbrace{(-2B^{-1}x+2d \alpha\widetilde\eta)^{*}}_{=\xi}(Bx + \lambda T \widetilde{\eta} + \lambda \widetilde I ).    
\end{aligned}
\end{equation}
...
\begin{aligned}\xi f  = & \big[-2 x^{*} x + 2 d \widetilde{\eta}^{*}  \alpha Bx\big] - 2 \lambda x^{*} B^{-1} T \widetilde{\eta} +\\ & +2 \lambda d  \widetilde{\eta}^{*} \alpha T \widetilde{\eta} - 2 \lambda x^{*} B^{-1} \widetilde I + 2\lambda d \widetilde\eta^{*} \alpha \widetilde I .\end{aligned}   
\end{equation*}
Using equation \eqref{2.3.5}, we derive the inequality:
\begin{equation*}\label{2.3.8} 
\begin{aligned} \xi f  \leqslant & \big[-2 \varepsilon b_m x^{*} x \big]- 2 \lambda x^{*} B^{-1} T \widetilde{\eta} +\\ & +2 \lambda d  \widetilde{\eta}^{*} \alpha T \widetilde{\eta} - 2 \lambda x^{*} B^{-1} \widetilde I + 2\lambda d \widetilde\eta^{*} \alpha \widetilde I.
\end{aligned}      
\end{equation*} 
Rewriting $-2\varepsilon b_m z^{*}z$ as the sum of two terms and multiplying the second term by $\lambda$, we obtain the following inequality (since $\lambda \in [0,1]$)
\begin{equation}\label{2.3.9}
\begin{aligned}\xi f \leqslant & - \varepsilon b_m x^{*} - \lambda \varepsilon b_m x^{*} x - 2 \lambda x^{*} B^{-1} T \widetilde{\eta} -\\ & -\big[ \lambda d \widetilde\eta^{*}\big (\alpha(-T) +(-T)^{*}\alpha\big)\widetilde\eta\big]- \\ &-2 \lambda x^{*} B^{-1} \widetilde I + 2\lambda d \widetilde\eta^{*} \alpha \widetilde I.
\end{aligned}
\end{equation}
Next, by adding and subtracting the term \\$\widetilde{\eta}^{*}\big(\frac{\lambda}{\varepsilon b_m}(B^{-1}T)^{*}(B^{-1}T)\big)\widetilde{\eta}$, we transform \eqref{2.3.9} into
\begin{multline}\label{2.3.10}
% \begin{aligned}
\xi f \leqslant  -\varepsilon b_m x^{*}x -\lambda\bigg\|\sqrt{\varepsilon b_m} x - \frac{1}{\sqrt{\varepsilon b_m}} B^{-1}T \widetilde{\eta} \bigg\|^2-\\
- \widetilde{\eta}^{*}\bigg( \lambda d \big(\alpha(-T)+(-T)^{*}\alpha\big) - \\
-\frac{\lambda}{\varepsilon b_m}(B^{-1}T)^{*}(B^{-1}T)\bigg) \widetilde{\eta} - \\
-2 \lambda x^{*} B^{-1} \widetilde I + 2\lambda d \widetilde\eta^{*} \alpha \widetilde I.
% \end{aligned}
\end{multline}
Applying condition \textbf{C3$'$}, we further transform \eqref{2.3.10} into
\begin{equation*}\label{2.3.11}
\xi f \leqslant -\varepsilon b_m x^{*} x - 2 \lambda x^{*} B^{-1} \widetilde{I} + 2 \lambda d \widetilde{\eta}^{*} \alpha  \widetilde{I}.
\end{equation*}
...
\begin{remark}[The idea of proof] This theorem can be proven using the LaSalle invariance principle, modified for regular Lyapunov functions \cite{ShevitzPaden1994}. To define the derivative of the Lyapunov function, the concept of the generalized gradient by Clarke and the chain rule theorem for regular functions can be used.
\end{remark}

\begin{IEEEproof} 
By Theorem \ref{exists}, the neural network \eqref{eq:special} possesses an equilibrium point (state).
Following the equilibrium shift performed in Section~\ref{sec:main}, let $e$ be the equilibrium of \eqref{eq:special} guaranteed by Theorem~\ref{exists}, and let $\eta' \in \overline{\operatorname{conv}}(g(e))$ satisfy $T\eta' = -(-Be + I)$ (see~\eqref{eq:outputequilib}).

\begin{remark}
{The existence of $\eta'$ can be established using reasoning analogous to that in the proof of Theorem \ref{exists}, i.e., by Makarov's theorem \cite{YLG}, for any solution $x(t)$, there exists a measurable bounded mapping $\eta(t) \in \overline{\operatorname{conv}}g(x(t))$ such that the equality in the inclusion holds almost everywhere.}
\end{remark}

Introducing $z = x - e$ exactly as in Section~\ref{sec:main}, we arrive at the shifted system (cf.~\eqref{sys:origin}):
\begin{equation}\label{2.4.1}
\dot z(t) \in -Bz+T~ \overline{\operatorname{conv}}(G(z)),
\end{equation}
where $G(z)=g(z+e)-\eta'$. As shown after \eqref{sys:origin}, the function $G$ inherits all relevant properties of $g$: $G \in \mathcal{G}'$, the origin is an equilibrium point of system~\eqref{2.4.1}, and the slope bound $\widetilde{\eta}_i / z \leqslant C_i$ holds for all $z \ne 0$ and all $\widetilde{\eta}_i \in \overline{\operatorname{conv}}[G_i(z)]$ (a consequence of Assumption~\textbf{C2}).
...
\]
Now, applying the chain rule for regular functions \cite{ShevitzPaden1994}, we express the derivative of $V(z(t))$ along the trajectories of system \eqref{2.4.1} as a scalar product:
\[
...
\begin{aligned} & \frac{\mathbf{d}V(z(t))}{\mathbf{d}t} = \big(-2B^{-1}z+2d\alpha \widetilde{\eta}\big)^{*}\big(Bz+T\widetilde{\eta}) =\\ 
% & = -2z^{*}z+2d\widetilde{\eta}^{*}\alpha B z - 2z^{*}B^{-1}T\widetilde{\eta} + 2d \widetilde{\eta}^{*} \alpha T \widetilde{\eta} =  \\
& = -2z^{*}z+2d\widetilde{\eta}^{*}\alpha B z -  2z^{*}B^{-1}T\widetilde{\eta} - \\
&~~~~~~~~~~~~~~~~~~~~~- \big[2d \widetilde{\eta}^{*} \alpha (-T) \widetilde{\eta} \big]= \\
& = -2z^{*}z+2d\widetilde{\eta}^{*}\alpha B z - 2z^{*}B^{-1}T\widetilde{\eta} - \\
& ~~~~~~~~~~~~~~~~~~~~~- \big[ d \widetilde{\eta}^{*}\big(\alpha(-T)+(-T)^{*}\alpha\big) \widetilde{\eta}\big] \leqslant\\
& \leqslant -2\varepsilon b_m z^{*}z - 2z^{*}B^{-1}T\widetilde{\eta} -\\&~~~~~~~~~~~~~~~~~~~~~- d \widetilde{\eta}^{*}\big(\alpha(-T)+(-T)^{*}\alpha\big) \widetilde{\eta}.
\end{aligned}
\end{equation}
We represent $-2\varepsilon b_m z^{*}z$ as two terms, then add and subtract $\widetilde{\eta}^{*}\big(\frac{1}{\varepsilon b_m}(B^{-1}T)^{*}(B^{-1}T)\big)\widetilde{\eta}$, transforming \eqref{2.4.5} into:
\begin{equation*}\label{2.4.6}
\begin{aligned} 
% & = -\varepsilon b_m z^{*}z  -\varepsilon b_m z^{*}z - 2z^{*}B^{-1}T\widetilde{\eta} + d \widetilde{\eta}^{*}\big(\alpha(-T)+(-T)^{*}\alpha\big) \widetilde{\eta} =\\
&\frac{\mathbf{d}V(z(t))}{\mathbf{d}t}  =  \\
&=-\varepsilon b_m z^{*}z -\bigg\|\sqrt{\varepsilon b_m} z - \frac{1}{\sqrt{\varepsilon b_m}} B^{-1}T \widetilde{\eta} \bigg\|^2-\\
& \quad- \widetilde{\eta}^{*}\bigg(d \big(\alpha(-T)+(-T)^{*}\alpha\big) - \\&~~~~~~~~~~~~~~~~~~~~~~~~~~~-\frac{1}{\varepsilon b_m}(B^{-1}T)^{*}(B^{-1}T)\bigg) \widetilde{\eta} \leqslant\\
& \leqslant-\varepsilon b_m z^{*}z - \Lambda \widetilde{\eta}^{*}\widetilde{\eta},
\end{aligned} 
\end{equation*}
...
The uniqueness of the equilibrium point can be shown analogously to the reasoning in \cite{eq_ref_11, periodic_time_varying, eq_ref_2}, where discontinuous activation functions for neural networks were also considered. Suppose that in the transformed system \eqref{2.4.1}, for which the origin is an equilibrium point, there exists another equilibrium point $z' \ne 0$, and $\eta'$ is the corresponding output equilibrium point (see Definition \ref{equilib}). Again by chain rule for regular functions~\cite{ShevitzPaden1994}, the expression for the derivative of the Lyapunov function along the system trajectories is:
\[
\frac{\mathbf{d}V(z')}{\mathbf{d}t} = \big(-2B^{-1}z'+2d\alpha {\eta'}\big)^{*}\big(Bz'+T {\eta}') = 0 .
\]
...
$\Delta_i := E(s^{(i)}) - E(s')$.
Write $E(s) = \frac{1}{2} s^* Q s - h^* s$ and
let $\delta := s^{(i)} - s'$.
Since only the $i$th component flips, we have
$\delta = -2 s_i' e_i$, where $e_i$ is the $i$th canonical basis vector.
Then
\begin{align*}
  \Delta_i
  &= \frac{1}{2}\bigl((s'+\delta)^* Q (s'+\delta)
      - (s')^* Q s'\bigr) - h^* \delta \\
  &= s'^{*} Q \delta + \frac{1}{2} \delta^* Q \delta - h^* \delta.
\end{align*}
Using $\delta_j = 0$ for $j \neq i$ and $\delta_i = -2 s_i'$, we obtain
\[
  s'^{*} Q \delta = (Q s')_i \delta_i,
  \qquad
  \delta^* Q \delta = \delta_i^2 Q_{ii} = 4 Q_{ii},
  \qquad
  h^* \delta = h_i \delta_i.
\]
Hence
\begin{align*}
  \Delta_i
  &= (Q s')_i (-2 s_i') + 2 Q_{ii} - h_i (-2 s_i') \\
  &= -2 s_i' (Q s')_i + 2 Q_{ii} + 2 h_i s_i'.
\end{align*}
Next expand $(Q s')_i = Q_{ii} s_i' + \sum_{j \neq i} Q_{ij} s_j'$.
Then
\[
  s_i' (Q s')_i
  = Q_{ii} + s_i' \sum_{j \neq i} Q_{ij} s_j',
...
\begin{align*}
  \Delta_i
  &= -2\Bigl[Q_{ii} + s_i' \sum_{j \neq i} Q_{ij} s_j'\Bigr]
     + 2 Q_{ii} + 2 h_i s_i' \\
  &= -2 s_i' \sum_{j \neq i} Q_{ij} s_j' + 2 h_i s_i' \\
  &= 2 s_i' \Bigl(h_i - \sum_{j \neq i} Q_{ij} s_j'\Bigr).
\end{align*}
Using $s_i' = \operatorname{sign}(h_i)$ and the strong field condition
...
flipping any single spin in $s'$ strictly increases the energy $E$.
Therefore $s'$ is a strict local minimizer of~$E$.

Now take any $s \in \{-1,1\}^n$ with $s \neq s'$.
There exists a finite sequence of single-spin flips transforming $s'$
into $s$:
\[
  s' = s^{(0)} \to s^{(1)} \to \dots \to s^{(k)} = s.
\]
At each step the energy strictly increases,
$E(s^{(\ell+1)}) > E(s^{(\ell)})$, $\ell=0,\dots,k-1$.
Hence
\[
  E(s) = E(s^{(k)}) > E(s^{(k-1)}) > \dots > E(s^{(0)}) = E(s'),
\]
which shows that $s'$ is the unique global minimizer of~$E$.
\end{IEEEproof}
...
\begin{equation}\label{eq:sign-activation}
s' = \operatorname{sign}(h).
\end{equation}

The activation~\eqref{eq:sign-activation} satisfies \textbf{C1--C2} and, by construction,
$T = -\beta Q_{\mathrm{shift}}$ with $Q_{\mathrm{shift}} > 0$ implies
$T+T^* < 0$, so that \textbf{C3} holds (with $\alpha=I$).
Therefore, by Theorem~\ref{thm:main}, system~\eqref{eq:hnn-qubo}
has a unique globally asymptotically stable Filippov equilibrium $x'$,
and any Filippov solution $x(\cdot)$ of~\eqref{eq:hnn-qubo} satisfies
$x(t) \to x'$ as $t \to +\infty$.

At a Filippov equilibrium one has
\[
  0 \in -B x' - \beta Q_{\mathrm{shift}} G' + \beta h,
\]
where $G'$ is the Filippov set of possible activation values at $x'$.
...
Hence no equilibrium can have $x_i' = 0$, and therefore
$g(x') = \operatorname{sign}(x')$ is single-valued and lies in
$\{-1,1\}^n$.

Denote $s := g(x') = \operatorname{sign}(x')$.
The equilibrium condition for~\eqref{eq:hnn-qubo} can be written as
\[
  0 = -B x' - \beta Q_{\mathrm{shift}} s + \beta h,
\]
that is,
\[
  x' = -B^{-1} \beta (Q_{\mathrm{shift}} s - h).
\]
...
consistent with such a relation is precisely $s'$.
Therefore $g(x') = s'$.

Finally, by Lemma~\ref{lem:qubo-unique}, $s'$ is the unique global minimizer
of the QUBO energy~\eqref{eq:qubo-energy}.
This completes the proof.
\end{IEEEproof}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Proofs for Application II (Penalty \& SVM)}\label{app:penalty-proofs}

This appendix supplies the details for Proposition~\ref{prop:SVM}. We use the
notation of Section~\ref{sec:penalty-HNN}. Throughout, \([r]_+:=\max\{0,r\}\)
and \(H(r)\) denotes the set-valued Heaviside graph,
\[
  H(r)=
  \begin{cases}
    \{0\}, & r<0,\\
  ...
  \end{cases}
\]
This convention is consistent with the Filippov regularization of the
discontinuous activation.

\subsection{Exact penalty and Hopfield realization}

Let the soft-margin SVM constraints be written as
\[
  r_i(u,\xi):=1-\xi_i-y_i u^* \bar z_i\le 0,
  \qquad
  s_i(\xi):=-\xi_i\le 0,
  \qquad i=1,\dots,m,
\]
where \(u=(w^*,b)^*\in\mathbb{R}^{d+1}\) and
\(\bar z_i=(z_i^*,1)^*\). To avoid degeneracies and to guarantee uniqueness
of the minimizer, we consider the regularized primal objective
\begin{equation}\label{eq:app-svm-primal-regularized}
  F_{\delta,\varepsilon}(u,\xi)
  :=
  \frac12\|w\|^2+\frac{\delta}{2}b^2
  +C\mathbf 1^*\xi+\frac{\varepsilon}{2}\|\xi\|^2,
  \qquad \delta,\varepsilon\ge 0.
\end{equation}
If \(\delta=\varepsilon=0\), the arguments below remain valid provided the
primal SVM solution is unique. If \(\delta>0\) and \(\varepsilon>0\), then
\(F_{\delta,\varepsilon}\) is strongly convex in \(x=(u^*,\xi^*)^*\).

The exact exterior penalty associated with the constraints is
\begin{equation}\label{eq:app-svm-exact-penalty}
  \Psi_\rho(u,\xi)
  :=
  F_{\delta,\varepsilon}(u,\xi)
  +\rho\sum_{i=1}^m [r_i(u,\xi)]_+
  +\rho\sum_{i=1}^m [-\xi_i]_+ .
\end{equation}
By the standard exact-penalty theorem for convex programs, if \(\rho\) is not
smaller than the largest optimal Lagrange multiplier of the SVM constraints,
then the minimizers of \(\Psi_\rho\) coincide with the minimizers of the
constrained SVM problem. In particular, under the uniqueness condition above,
\(\Psi_\rho\) has a unique minimizer \(x'=(u',\xi')\).

The subdifferential of \(\Psi_\rho\) has the form
\begin{equation}\label{eq:app-svm-subdiff}
  \partial \Psi_\rho(x)
  =
  Qx+c
  +\rho\sum_{i=1}^m H(r_i(x))\,\nabla r_i
  +\rho\sum_{i=1}^m H(-\xi_i)\,\nabla(-\xi_i),
\end{equation}
where \(Q=\operatorname{diag}(I_d,\delta,\varepsilon I_m)\succeq 0\) and
\(c=(0,\dots,0,C\mathbf 1^*)^*\) collects the linear term in \(\xi\).
The vectors \(\nabla r_i\) and \(\nabla(-\xi_i)\) are constant. Hence
\eqref{eq:app-svm-subdiff} is a Lur'e-type expression with piecewise-constant
set-valued nonlinearities \(H(r_i(x))\) and \(H(-\xi_i)\).

The associated Hopfield/subgradient dynamics is
\begin{equation}\label{eq:app-svm-subgradient-flow}
  \dot x(t)\in -\partial\Psi_\rho(x(t)).
\end{equation}
Equivalently, after collecting the affine terms and, if desired, adding a
diagonal shift \(\mu I\) to the linear part, \eqref{eq:app-svm-subgradient-flow}
can be written in the discontinuous HNN form
\[
  \dot x=-Bx+Tg(x)+I,
\]
where \(g\) is composed of Heaviside graphs. Since each component of \(g\) is
bounded and has a single upward jump, C1 and C2 hold. Choosing the shift so that
the symmetric part of the effective interconnection is negative definite gives
\(T+T^*<0\), hence the diagonal stability condition C3 holds with the identity
diagonal multiplier.

\subsection{Proof of Proposition~\ref{prop:SVM}}

\begin{proof}
Let \(x'=(u',\xi')\) be the unique minimizer of the exact penalty
\(\Psi_\rho\) in \eqref{eq:app-svm-exact-penalty}. By convexity,
\[
  x'\in\operatorname*{arg\,min}\Psi_\rho
  \quad\Longleftrightarrow\quad
  0\in \partial\Psi_\rho(x').
\]
Thus \(x'\) is an equilibrium of the subgradient/Hopfield inclusion
\eqref{eq:app-svm-subgradient-flow}.

We now prove global convergence. Let
\[
  W(x):=\Psi_\rho(x)-\Psi_\rho(x')\ge 0 .
\]
Along any absolutely continuous solution of
\eqref{eq:app-svm-subgradient-flow}, choose
\(\eta(t)\in\partial\Psi_\rho(x(t))\) such that
\(\dot x(t)=-\eta(t)\) for a.e. \(t\). The chain rule for convex functions
gives
\[
  \frac{d}{dt}\Psi_\rho(x(t))
  =
  \langle \eta(t),\dot x(t)\rangle
  =
  -\|\eta(t)\|^2
  =
  -\|\dot x(t)\|^2
  \le 0
\]
for a.e. \(t\). Hence \(W\) is nonincreasing along all Filippov solutions.

If \(\Psi_\rho\) is strongly convex, then for some \(\sigma>0\),
\[
  \langle \eta-\eta',x-x'\rangle\ge \sigma\|x-x'\|^2,
  \qquad
  \eta\in\partial\Psi_\rho(x),\quad
  \eta'\in\partial\Psi_\rho(x').
\]
Taking \(\eta'=0\) yields
\[
  \langle \eta,x-x'\rangle\ge \sigma\|x-x'\|^2 .
\]
For the quadratic distance \(Y(x)=\frac12\|x-x'\|^2\), we obtain
\[
  \dot Y
  =
  \langle x-x',\dot x\rangle
  =
  -\langle x-x',\eta\rangle
  \le
  -\sigma\|x-x'\|^2 .
\]
Therefore \(x(t)\to x'\) exponentially, and in particular globally
asymptotically, from every initial condition. In the nonregularized case
\(\delta=\varepsilon=0\), the same conclusion follows from the YLG theorem of
the main text under C1--C3 and the assumed uniqueness of the equilibrium.

It remains to identify \(x'\) with the SVM solution. Since \(\rho\) is chosen
large enough for exactness, the minimizers of \(\Psi_\rho\) coincide with the
minimizers of the constrained SVM problem. Hence \(x'=(u',\xi')\) is the unique
primal SVM optimizer. Writing \(u'=(w'^*,b')^*\), the classifier parameters
\((w',b')\) are precisely the optimal soft-margin SVM parameters. This proves
Proposition~\ref{prop:SVM}.
\end{proof}

\begin{remark}
The terms \(H(r_i(x))\) and \(H(-\xi_i)\) encode active margin and nonnegativity
constraints. They are the discontinuous activations in the Hopfield
realization. The diagonal shift does not alter the subgradient-flow
interpretation when it is incorporated into the quadratic Lyapunov/YLG
functional; its role is to enforce the negative symmetric part required in C3.
\end{remark}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Proofs and Derivations for Application III: Real-Time Constrained Control}
\label{app:control-proofs}

This appendix provides the proof of Proposition~\ref{prop:rt-solver} and the
derivations for the one- and two-dimensional examples.

\subsection{Proof of Proposition~\ref{prop:rt-solver}}

Let
\[
  K:=[u_{\min},u_{\max}]
\]
be the control box. For a fixed plant state \(z\), the constrained QP is
\begin{equation}\label{eq:app-control-qp}
  \min_{u\in K}
  J_z(u):=\frac12 u^*Hu+f(z)^*u,
  \qquad H=H^*>0 .
\end{equation}
Equivalently,
\[
  ...
  \widehat J_z(u):=
  \frac12 u^*Hu+f(z)^*u+\iota_K(u).
\]
Since \(H>0\), \(\widehat J_z\) is proper, lower semicontinuous, and strongly
convex. Hence it has a unique minimizer \(u'(z)\), characterized by
\begin{equation}\label{eq:app-control-kkt}
  0\in Hu'(z)+f(z)+N_K(u'(z)).
\end{equation}

To match the notation of Section~\ref{sec:application_control}, choose a
diagonal leakage matrix \(A>0\) such that \(H-A\succeq 0\) and set
\begin{equation}\label{eq:app-control-phi-choice}
  \phi(x):=\frac12 x^*(H-A)x+\iota_K(x),
  \qquad
  b(z):=-f(z).
\end{equation}
Then the parameterized YLG/subgradient functional
\[
  V_z(x):=\phi(x)-b(z)^*x+\frac12 x^*Ax
\]
satisfies
\begin{equation}\label{eq:app-control-V-equals-J}
  V_z(x)
  =
  \frac12 x^*Hx+f(z)^*x+\iota_K(x)
  =
  \widehat J_z(x).
\end{equation}
Thus
\[
  \partial V_z(x)=Hx+f(z)+N_K(x),
\]
and the HNN dynamics
\begin{equation}\label{eq:app-control-flow}
  \dot x(t)\in-\partial V_z(x(t))
\end{equation}
is precisely the subgradient flow of the strongly convex constrained QP
objective.

The equilibrium condition for \eqref{eq:app-control-flow} is
\[
  0\in\partial V_z(x'),
\]
which, by \eqref{eq:app-control-kkt}--\eqref{eq:app-control-V-equals-J}, is
equivalent to \(x'=u'(z)\). Therefore the HNN equilibrium is the unique
optimizer of \eqref{eq:app-control-qp}.

It remains to show global convergence. Let
\[
  W_z(x):=V_z(x)-V_z(x'(z))\ge 0 .
\]
For any solution of \eqref{eq:app-control-flow}, choose
\(\eta(t)\in\partial V_z(x(t))\) such that \(\dot x(t)=-\eta(t)\) a.e. The
convex chain rule gives
\[
  \dot W_z(x(t))
  =
  \langle \eta(t),\dot x(t)\rangle
  ...
  \le 0 .
\]
Since \(V_z\) is strongly convex, \(\partial V_z\) is strongly monotone:
there exists \(\lambda_H>0\) such that
\[
  \langle \eta-\eta',x-y\rangle
  \ge \lambda_H\|x-y\|^2,
  \qquad
  \eta\in\partial V_z(x),\quad \eta'\in\partial V_z(y).
\]
Taking \(y=x'(z)\) and \(\eta'=0\), one obtains
\[
  \frac{d}{dt}\frac12\|x(t)-x'(z)\|^2
  =
  -\langle x(t)-x'(z),\eta(t)\rangle
  \le
  -\lambda_H\|x(t)-x'(z)\|^2 .
\]
Hence \(x(t)\to x'(z)\) exponentially for each frozen \(z\). In particular,
\(x'(z)\) is globally asymptotically stable. This proves
Proposition~\ref{prop:rt-solver}.

Finally, the inclusion \eqref{eq:app-control-flow} may equivalently be written
as the projected dynamical system
\[
  \dot x
  =
  \Pi_{T_K(x)}
  \bigl(-Hx-f(z)\bigr),
\]
where \(T_K(x)\) is the tangent cone of \(K\) at \(x\). This projected form
makes explicit that the normal cone implements the discontinuous saturation
needed to keep trajectories viable in the box.
\end{proof}

\subsection{Derivations for the 1D example}

Consider
\[
  \dot z=-z+u,\qquad |u|\le 1,
\]
and
\begin{equation}\label{eq:app-1d-cost}
  J_z(u)=\frac12 h u^2+\alpha z u,
  \qquad h>0,\quad \alpha>0.
\end{equation}
The unconstrained minimizer is
\[
  u_{\rm unc}'(z)=-\frac{\alpha}{h}z,
\]
and the constrained optimizer is its projection onto the admissible interval:
\begin{equation}\label{eq:app-1d-projection}
  u'(z)
  =
  \Pi_{[-1,1]}\!\left(-\frac{\alpha}{h}z\right).
\end{equation}

The nonsmooth constrained objective is
\[
  \widehat J_z(x)
  =
  \frac12 h x^2+\alpha zx+\iota_{[-1,1]}(x),
\]
with
\[
  \partial \widehat J_z(x)
  =
  hx+\alpha z+N_{[-1,1]}(x).
\]
The corresponding HNN/subgradient dynamics is therefore
\begin{equation}\label{eq:app-1d-hnn-correct}
  \dot x\in -hx-\alpha z-N_{[-1,1]}(x).
\end{equation}
Equivalently, if the main text writes the dynamics with \(+\alpha z\), then the
cost must be written as \(\frac12 h u^2-\alpha z u\). For the cost
\eqref{eq:app-1d-cost}, \eqref{eq:app-1d-hnn-correct} is the sign-consistent
form.
...
\[
  0\in hx'+\alpha z+N_{[-1,1]}(x'),
\]
which is equivalent to
\[
  x'=u'(z)=\Pi_{[-1,1]}\!\left(-\frac{\alpha}{h}z\right).
\]

For fixed \(z\), define
\[
  W_z(x):=\widehat J_z(x)-\widehat J_z(x'(z)).
\]
Along \eqref{eq:app-1d-hnn-correct}, choosing
\(\eta(t)\in\partial\widehat J_z(x(t))\) with
\(\dot x(t)=-\eta(t)\), we get
\[
  \dot W_z(x(t))
  =
  -|\eta(t)|^2
  ...
  \le 0 .
\]
Since \(h>0\), the objective is strongly convex, so the equilibrium is unique
and globally asymptotically stable.

For the full closed-loop system
\[
  \dot z=-z+x,\qquad
  \dot x\in -hx-\alpha z-N_{[-1,1]}(x),
\]
...
\[
  z'=x',
  \qquad
  x'=\Pi_{[-1,1]}\!\left(-\frac{\alpha}{h}z'\right).
\]
Thus \(x'=z'\) and
\[
  x'=\Pi_{[-1,1]}\!\left(-\frac{\alpha}{h}x'\right).
\]
For \(x'\in(-1,1)\), this gives
\[
  \left(1+\frac{\alpha}{h}\right)x'=0,
\]
hence \(x'=0\). At \(x'=1\), the right-hand side is
\(\Pi_{[-1,1]}(-\alpha/h)<1\); at \(x'=-1\), it is
\(\Pi_{[-1,1]}(\alpha/h)>-1\). Therefore no boundary equilibrium exists and
\[
  (z',x')=(0,0)
\]
is the unique closed-loop equilibrium.

\subsection{Derivations for the 2D example}
...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
where the subscript \(p\) is used here to avoid confusion with the leakage
matrix in the Hopfield equation. Consider
\begin{equation*}
  \begin{aligned}
  \dot z&=A_pz+B_pu,
   J_z(u)=\frac12 u^*Hu+q(z)^*u,
  \qquad\\
  H&=\operatorname{diag}(2,1)>0,
  \qquad
  q(z)=
  \begin{bmatrix}
    z_1+0.5z_2\\
...
\[
  ...
  \end{bmatrix},
\]
and the constrained optimizer is
\begin{equation}\label{eq:app-2d-projection}
  u'(z)=\Pi_{[-1,1]^2}\bigl(-H^{-1}q(z)\bigr).
\end{equation}

The constrained objective is
\[
  \widehat J_z(x)=\frac12 x^*Hx+q(z)^*x+\iota_{[-1,1]^2}(x),
\]
and
\[
  \partial\widehat J_z(x)=Hx+q(z)+N_{[-1,1]^2}(x).
\]
Therefore the sign-consistent HNN/subgradient flow is
\begin{equation}\label{eq:app-2d-hnn-correct}
  \dot x\in -Hx-q(z)-N_{[-1,1]^2}(x).
\end{equation}
More generally, if an additional diagonal leakage \(A_\ell>0\) is included in
the network, then exact equivalence with the original QP is retained by
replacing the quadratic part in the auxiliary potential by \(H-A_\ell\), as in
\eqref{eq:app-control-phi-choice}. The resulting total Lyapunov functional is
still \(\widehat J_z\), and the equilibrium remains \eqref{eq:app-2d-projection}.

The normal cone to the rectangle is the Cartesian product
\[
  N_{[-1,1]^2}(x)=N_{[-1,1]}(x_1)\times N_{[-1,1]}(x_2).
\]
The equilibrium condition for fixed \(z\) is
\[
  0\in Hx'+q(z)+N_{[-1,1]^2}(x'),
\]
which is exactly the optimality condition for the box-constrained QP. Hence
\(x'(z)=u'(z)\).

For convergence, define
\[
  W_z(x)=\widehat J_z(x)-\widehat J_z(x'(z)).
\]
Along \eqref{eq:app-2d-hnn-correct},
\[
  \dot W_z(x(t))=-\|\eta(t)\|^2\le 0,
  \qquad
  \eta(t)\in\partial\widehat J_z(x(t)),\quad
  \dot x(t)=-\eta(t).
\]
Because \(H>0\), the objective is strongly convex; consequently,
\[
  \frac{d}{dt}\frac12\|x(t)-x'(z)\|^2
  \le
  -\lambda_{\min}(H)\|x(t)-x'(z)\|^2 .
\]
Thus \(x'(z)\) is globally exponentially stable for each frozen \(z\), and
therefore globally asymptotically stable.

At a steady state of the full closed-loop system one has
\[
  0=A_pz'+x',
  \qquad
  x'=\Pi_{[-1,1]^2}\bigl(-H^{-1}q(z')\bigr).
\]
Since \(A_p\) is nonsingular,
\[
  x'=-A_pz'.
\]
Substitution into the variational inequality for the projection gives the
closed-loop equilibrium condition. In particular, \(z'=0\) implies \(x'=0\),
and the strong monotonicity of the QP map together with the Hurwitz character
of \(A_p\) rules out additional equilibria in the small-gain regime used in the
example. Hence the origin is the unique closed-loop equilibrium for the
parameter choice stated in Section~\ref{subsec:2d-example-control}.
