...
% *** 20.04.2026 
%remove repeating Lur'e form equations 
% ^{*} -> '
%\ast -> '
% ^\star -> '
% fix eq ref eq:sign-activation eq:strong-field and others
...
and \emph{YLG-type frequency condition} --- under which the network admits a \emph{unique equilibrium} 
...
\begin{align*}
  ...
  &= \frac{1}{2}\bigl((s'+\delta)^* Q (s'+\delta)
      - (s')^* Q s'\bigr) - h^* \delta \\
  &= s^{\star^*} Q \delta + \frac{1}{2} \delta^* Q \delta - h^* \delta.
\end{align*}
Using $\delta_j = 0$ for $j \neq i$ and $\delta_i = -2 s_i'$, we obtain
\[
  s^{\star^*} Q \delta = (Q s')_i \delta_i,
  \qquad
...
\ref{eq:strong-field}, we estimate
\[
  \Bigl| \sum_{j \neq i} Q_{ij} s_j' \Bigr|
  \leq \sum_{j \neq i} |Q_{ij}|
  < |h_i|.
