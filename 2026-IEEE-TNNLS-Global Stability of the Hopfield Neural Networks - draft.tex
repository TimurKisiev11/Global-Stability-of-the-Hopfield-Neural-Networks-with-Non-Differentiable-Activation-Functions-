...
\emph{non-differentiable, possibly non-monotone and unbounded} activations. 
...
We further embed MAX-CUT, TSP, N-Queens, and soft-margin SVM into this discontinuous-activation Hopfield template 
and verify that each satisfies C1--C3, with simulations confirming convergence from arbitrary initial conditions. 
...
\item \textbf{Optimization embeddings.} We demonstrate that standard penalty embeddings of
      MAX-CUT, TSP, $N$-Queens, and soft-margin SVM satisfy \textbf{C1--C3}, and we provide numerical
      integrations consistent with the theory.
\end{enumerate}
...
Section~\ref{sec:problem} states the HNN model in the Lur\'{e} form and assumptions~C1--C3.
Section~\ref{sec:main} presents the main results on uniqueness and GAS and discusses tightness.
Section~\ref{sec:examples} contains some numerical illustrations, followed by Sections~\ref{sec:QUBO} and \ref{sec:penalty-HNN} that verify C1--C3 for four optimization embeddings,
...
\end{equation}
 ...
 of the system~\eqref{eq:special} as a stationary solution
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Extended Nonlinearities in Lur'e Systems (after YLG)}
...
% ------------------------------------------------
\begin{assumption}[\bf C3: Diagonal Lyapunov stability (alternative)]\label{ass:C3}
...
\textbf{C3$'$} if one prefers to avoid frequency-domain inequalities (see Appendix \ref{app:proofs}).
...
\todo[inline]{NEW BELOW}
\section{Nonunique Filippov trajectories under assumptions C1–C3:
A Low–Dimensional Example}\label{sec:nonunique}

In this section we present a low-dimensional example demonstrating that, even when the standard assumptions C1--C3 (linear growth, extended nonlinearity structure, and diagonal stability of the linear part) are satisfied, the corresponding Filippov inclusion may admit multiple distinct forward trajectories starting from the same initial state. This shows that global asymptotic stability of the unique equilibrium does not, in general, imply forward uniqueness of Filippov solutions.
...
The system \eqref{eq:example-system} satisfies conditions C1--C3 for $|a|<2$:
\begin{itemize}
...
Now let’s take function $s(x)$ (Fig. \ref{fig4:left}) as activation for the system $(\ref{example1})$. Assumptions C1 (linear growth) and C2 (unilateral Lipschitz-like condition) fail because, as stated in~\cite{unilateral_Lipschitz_like_condition}, this function possesses super linear growth and does not comply with unilateral Lipschitz-like condition. The restriction condition from definition \ref{G_n} is also violated $s_i((-1)^+) = 0.01 < 3 = s_i((-1)^-)$ and therefore this function does not belong to the class $\mathcal{G'}$ (Definition \ref{G_n}). 
...
Lastly, let's present the counterexample to assumption C3 (Diagonal Lyapunov stability) by taking a skew-symmetric matrix $T$ ($T^{\mathrm{T}} = -T$):
...
\end{equation}
Then \(T=-\beta Q_{\mathrm{shift}}\) yields \(T+T^\top\prec0\), so C3 holds (with \(\alpha=I\)); C1–C2 hold for the sign activation.
\begin{theorem}[Global convergence to the planted optimizer]\label{th:hnn-solves-strong-field-qubo}
Under the strong field condition and C1–C3, \eqref{eq:hnn-qubo} has a unique Filippov equilibrium \(x^\star\), globally asymptotically stable, and \(g(x^\star)=\mathrm{sign}(x^\star)=s^\star\), the unique QUBO optimizer. 
Proof in Appendix~\ref{app:qubo-proofs}.
\end{theorem}
...
satisfying C1–C3 after a standard diagonal shift of the quadratic part (YLG/Lur’e spirit). 
...
Let \(x=(u^\top,\xi^\top)^\top\). The piecewise-linear kinks in the hinge term define a componentwise discontinuous activation (single upward jump). Choosing a diagonal leakage and a shifted quadratic synaptic matrix (so \(T+T^\top\prec0\)) yields a discontinuous HNN of the Lur’e form satisfying C1–C3. Therefore:
...
\end{equation}
This is a Filippov inclusion with finitely many first-order discontinuities (due to the box normal cone). It fits the Lur’e form \(\dot x=-Bx+Tg(x)+I\) and satisfies C1–C3 (linear growth, one-sided slope, diagonal stability) under the choices below.

\begin{proposition}[Real-time solver under C1–C3]\label{prop:rt-solver}
Under Assumptions~C1–C3 and the model \eqref{eq:hnn-subgrad}–\eqref{eq:Vz-def}, for each frozen \(z\) the HNN admits a unique Filippov equilibrium \(x^\ast(z)\), globally asymptotically stable, and \(x^\ast(z)=u^\ast(z)\) solves \eqref{eq:opt-problem}. \emph{(Proof in Appendix~\ref{app:control-proofs}.)}
\end{proposition}
...
Then \(\dot V_z\le -x^\top A_\ell x\), hence C1–C3 hold and \(x^\ast(z)=u^\ast(z)\) globally. 
...
Now, based on assumptions \textbf{C2} and \textbf{C3}, we formulate Lemma 1 which will be used both in Theorem 2 and 3 to prove existence and stability of the equilibrium point. It can be considered an attempt to rewrite these assumptions to align proofs presented in this work with those presented by M. Forti and P. Nistiri \cite{p1}. Since the proof of the Lemma consists only of trivial mathematical transformations and directly follows from properties \textbf{C2} and \textbf{C3}, we do not provide it in this work.  \\
% (CHANGE d -> c to better align)
\begin{lemma}\label{main_lemma}
    \textnormal{Let assumptions \textbf{C2} and \textbf{C3} hold, then there exist positive $d$ and $\varepsilon$ such that:}
...
The activation~\eqref{eq:sign-activation} satisfies C1--C2 and, by construction,
$T = -\beta Q_{\mathrm{shift}}$ with $Q_{\mathrm{shift}} \succ 0$ implies
$T+T^\top \prec 0$, so that C3 holds (with $\alpha=I$).
