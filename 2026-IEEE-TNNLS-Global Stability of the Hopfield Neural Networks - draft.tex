...
satisfied for~\eqref{eq:special_conv} in view of Assumptions~C1–C2, every initial condition admits at least one global
...
Consider the Hopfield neural network (HNN) in the form \eqref{eq:special}, where $g(x)=(g_1(x_1),\ldots,g_n(x_n))^*$ is the componentwise activation
...
\[
  \varphi(\sigma)
  := \overline{\mathrm{conv}}\,g(\sigma)
  = \bigl(\overline{\mathrm{conv}}\,g_1(\sigma_1),\dots,
          \overline{\mathrm{conv}}\,g_n(\sigma_n)\bigr)^* .
\]
...
\begin{equation}\label{eq:C3-matrix}
  -HB - B^{*}H + \alpha\bigl(T^{*}H + HT\bigr) \prec 0 .
\end{equation}
\end{assumption3mat}
...
$H_{\mathrm{YLG}}\succ 0$, we have $x^{*}H_{\mathrm{YLG}}Bx>0$ for
...
$\alpha'>0$ such that $S(\alpha)\prec 0$ for all
...
\begin{equation*}
   ...
   \qquad \varphi(x) = \overline{\mathrm{conv}}\,g(x).
\end{equation*}
...
\end{equation}
with $P=-B$, $q=T$, $r^{*}=I_n$ and $\varphi=\overline{\mathrm{conv}}\,g$ as
...
\end{equation}
Then \(T=-\beta Q_{\mathrm{shift}}\) yields \(T+T^*\prec0\), so \textbf{C3} holds (with \(\alpha=I\)); \textbf{C1-C2} hold for the sign activation.
...
\end{equation}
then \((Q^{\mathrm{MC}},h)\) meets Definition~\ref{eq:strong-field}. With \(Q_{\mathrm{shift}}=Q^{\mathrm{MC}}+\mu I\succ0\) and the HNN \eqref{eq:hnn-qubo}, Theorem~\ref{th:hnn-solves-strong-field-qubo} implies:
\begin{corollary}\label{cor:hnn-planted-maxcut}
The HNN output converges globally to \(s'\) aligned with the planted partition \((A,B)\), which is the unique minimizer of the MAX-CUT QUBO with field. Proof in Appendix~\ref{app:qubo-proofs}.
\end{corollary}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{equation}\label{eq:penalty-V}
  V(x)=\phi(x)+\tfrac12 x^* A x - \ell^* x,\qquad A\succ0,
\end{equation}
...
Let \(x=(u^*,\xi^*)^*\). The piecewise-linear kinks in the hinge term define a componentwise discontinuous activation (single upward jump). Choosing a diagonal leakage and a shifted quadratic synaptic matrix (so \(T+T^*\prec0\)) yields a discontinuous HNN of the Lur’e form satisfying \textbf{C1-C3}. Therefore:
...
Under Assumptions~\textbf{C1--C3} and the model \eqref{eq:hnn-subgrad}–\eqref{eq:Vz-def}, for each frozen \(z\) the HNN admits a unique Filippov equilibrium \(x'(z)\), globally asymptotically stable, and \(x'(z)=u'(z)\) solves \eqref{eq:opt-problem}. \emph{(Proof in Appendix~\ref{app:control-proofs}.)}
\end{proposition}
...
$T+T^* \prec 0$, so that \textbf{C3} holds (with $\alpha=I$).
